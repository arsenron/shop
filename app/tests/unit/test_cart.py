...
def test_same_kind_rule():
    shopping_cart = ShoppingCart(
        cart_products=[CartProduct(amount=5, product=Product(id=2, name="orange", price=4))]
    )
...
def test_exceeding_rule():
    shopping_cart = ShoppingCart(
        cart_products=[CartProduct(amount=3, product=Product(id=2, name="orange", price=20))]
    )
    with pytest.raises(ShoppingCartError):
        ExceedingRule(total_sum=50).apply_rule(shopping_cart)
...
def test_discount_rule():
    ...
    discount_rule = DiscountRule(discount_amount=1, total_sum=20)
    discount_rule.apply_rule(shopping_cart)
    assert shopping_cart.total_amount == 59

    shopping_cart = ShoppingCart(
        cart_products=[CartProduct(amount=3, product=Product(id=2, name="orange", price=6))]
    )
    discount_rule = DiscountRule(discount_amount=1, total_sum=20)
    discount_rule.apply_rule(shopping_cart)
